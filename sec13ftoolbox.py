...
PG_COLS = (
    ...
    346.4,
    465.47,
)  # First two divs separate out the cusip into three pieces
...
def scrub_lis(http_url, selector):
    ...
    if selector is None:
        ...
        if not bool(re.search("\d{4}", year)) & bool(re.search("[1-4]", quarter)):
            print(
                "specified selector '{SELECTOR}' does not follow [yyyy]q[q] format".format(
                    SELECTOR=selector
                )
            )
            exit()
        print("Looking for {YEAR}/{QUARTER}...".format(YEAR=year, QUARTER=quarter))
        selector_pattern = r"{QUARTER}.. quarter {YEAR}".format(
...
def pdf2df(filename):
    ...
    print(
        "{FILENAME}: extracting pages:{PG_START}-{PG_END}".format(
            FILENAME=filename, PG_START=PG_START_INDEX, PG_END=num_pages
        )
    )
    ...
    expected_row_count = int(
        "".join(re.findall(r"\d", (str(df.iloc[len(df) - 1][len(TBL_HEADER) - 1]))))
    )
...
@click.command()
@click.option("--file", "-f", help="specify local pdf securities file location")
@click.option(
    "--selector", "-s", help="specify year and quarter of the report, ie 2018q4"
)
@click.option(
    "--dir",
    "-d",
...
@click.option("--on", "-o", help="join on column name", default="CUSIP - Current")
def main(file, selector, dir, on):
    ...
    if dir is not None:
        if not os.path.isdir(dir):
            print(
                "Input join directory {DNAME} does not exist. Exiting...".format(
                    DNAME=dir
                )
            )
            quit()
            ...
            in_dfs = [pd.read_csv(f) for f in files]
            print(
                "Read in {DFS} source reports. Will join on '{ON}' column.".format(
                    DFS=len(in_dfs), ON=on
                )
            )
    ...
    if file is not None:
        ...
        if not os.path.isfile(file):
            ...
            filename = file
            filename_csv = filename.removesuffix(".pdf") + ".csv"
            ...
            year = int(basename[
                basename.index("13flist") + len("13flist") : basename.index("q")
            ])
            quarter = int(basename[basename.index("q") + 1])
            ...
            df["has_listed_options"] = df["has_listed_options"].apply(lambda x: len(x.strip()) > 0 if type(x) is str else False)
            df["year"] = year
            df["quarter"] = quarter
            df.info()
            ...
            for left_df in in_dfs:
                print(
                    "Inner JOIN '{LEFT}' '{RIGHT}' ON '{ON}'".format(
                        LEFT=files[i], RIGHT=filename, ON=on
                    )
                )
                (
                    left_df.merge(df, left_on=on, right_on=TBL_HEADER[0], how="inner")
                ).to_csv(files[i] + "-" + filename_csv, index=False, header=True)
                i = i + 1
